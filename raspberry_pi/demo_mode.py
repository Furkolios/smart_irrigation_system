...
def generate_mock_weather(rain_chance: float = 0.2) -> Dict[str, Any]:
    ...
    return {
        ...
        tomorrow: make_day(tomorrow, random.uniform(0, 5), random.random() < 0.3, temp_base + 1)
    }
...
class DemoMode:
    ...
    ZONE_NAMES = [
        "Tomato Bed", "Herb Garden", "Lettuce Row",
        "Pepper Plants", "Squash Patch", "Berry Bushes"
    ]
    ...
    def _init_demo(self):
        ...
        self.engine = IrrigationDecisionEngine(
            zones=self.zones,
            params=demo_params
        )
    ...
    def _apply_irrigation(self, result: DecisionResult):
        ...
        for cmd in result.commands:
            if cmd.zone_id in self.zone_states:
                ...
                state.moisture_percent = min(90, state.moisture_percent + moisture_increase)
...
if __name__ == "__main__":
    ...
    for scenario in ['normal', 'critical', 'rain', 'healthy', 'low_tank', 'mixed']:
        ...
        for zone in state['zones']:
            ...
            print(f"{zone['name'].split()[0]}:{zone['moisture_percent']:.0f}%{status}", end=" ")
        print()
        
        print(f"Decision: ", end="")
        if result.should_irrigate:
            print(f"Irrigate {len(result.commands)} zone(s), {result.total_water_liters:.1f}L total")
        else:
