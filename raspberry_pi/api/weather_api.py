...
class CacheManager:
    ...
    def __init__(
        ...
        duration_minutes=DEFAULT_CACHE_DURATION_MINUTES,
        logger=None,
    ):
...
class WeatherAPI:
    ...
    def __init__(
        ...
        if not api_key:
            raise ValueError(
                "API key is required. Set OPENWEATHER_API_KEY environment variable or pass directly."
            )
        ...
        self.logger = setup_logger(
            name="smart_irrigation", level=log_level, log_file=log_file, silent=silent
        )
    ...
    def get_current_weather(self, city):
        ...
        data = self._fetch_data(
            self.base_weather_url, params, f"current_{city.lower()}"
        )
    ...
    def get_forecast(self, city):
        params = {"q": city, "appid": self.api_key, "units": "metric"}
        data = self._fetch_data(
            self.base_forecast_url, params, f"forecast_{city.lower()}"
        )
...
def create_api(api_key=None, cache_enabled=True, log_file=None, silent=False):
    ...
    return WeatherAPI(
        api_key=key, cache_enabled=cache_enabled, log_file=log_file, silent=silent
    )
