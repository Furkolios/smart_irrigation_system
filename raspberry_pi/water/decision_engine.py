...
class IrrigationDecisionEngine:
    ...
    def set_plant_data(self, zone_id: str, plant_data: Dict[str, Any]) -> None:
        ...
        if zone_id in self.zones and plant_data:
            ...
            if plant_data.get("drought_tolerant"):
                zone.moisture_threshold_low = max(
                    20.0, zone.moisture_threshold_low - 10
                )
                zone.moisture_threshold_target = max(
                    40.0, zone.moisture_threshold_target - 10
                )
    ...
    def _check_global_constraints(
        ...
        if has_critical:
            ...
            if hour < self.params["allowed_hours_start"]:
                return (
                    False,
                    f"Too early ({hour}:00) - waiting until {self.params['allowed_hours_start']}:00",
                )
            ...
            if (
                self.params["avoid_midday_start"]
                <= hour
                < self.params["avoid_midday_end"]
            ):
                return (
                    False,
                    f"Avoiding midday evaporation ({hour}:00) - waiting until {self.params['avoid_midday_end']}:00",
                )
    ...
    def _calculate_urgency_score(
        self, reading: SensorReading, weather: Dict[str, Any]
    ) -> float:
        """
    ...
    def _calculate_water_amount(
        self, zone: ZoneConfig, urgency_score: float, weather: Dict[str, Any]
    ) -> float:
        """
    ...
    def _build_priority_queue(
        self, sensor_data: Dict[str, SensorReading], weather: Dict[str, Any]
        ...
        for zone_id, reading in sensor_data.items():
            ...
            if urgency < self.params["urgency_threshold_low"]:
                self.logger.debug(
                    f"Zone {zone_id}: urgency {urgency:.1f} below threshold, skipping"
                )
                continue
    ...
    def _build_reason(
        ...
        if reading.soil_moisture_percent < zone.moisture_threshold_low:
            ...
            parts.append(
                f"moisture below target ({reading.soil_moisture_percent:.0f}%)"
            )
    ...
    def _allocate_water(
        ...
        reserve_liters = tank_status.capacity_liters * (
            self.params["tank_reserve_percent"] / 100
        )
        emergency_reserve = tank_status.capacity_liters * (
            self.params["emergency_reserve_percent"] / 100
        )
...
def quick_decision(
    ...
    # Create tank status
    tank = TankStatus(
        current_level_liters=tank_level_liters, capacity_liters=tank_capacity_liters
    )
...
if __name__ == "__main__":
    ...
    engine.set_plant_data(
        "zone_1",
        {"common_name": "Tomato", "watering": "Frequent", "drought_tolerant": False},
    )
    engine.set_plant_data(
        "zone_2",
        {"common_name": "Mixed Herbs", "watering": "Minimum", "drought_tolerant": True},
    )
    engine.set_plant_data(
        "zone_3",
        {"common_name": "Lettuce", "watering": "Average", "drought_tolerant": False},
    )
    ...
    weather_data = {
        today: {
            ...
            "temperature": {"day_min": 15.0, "day_max": 28.0, "by_period": {}},
            "et": {"et_mm": 4.5, "temp_avg": 22.0, "humidity_avg": 55.0},
            "water_balance_mm": -2.5,
        }
    }

    # Simulate tank status
    tank = TankStatus(current_level_liters=80.0, capacity_liters=100.0)
    ...
    print(
        f"Tank after: {result.tank_after_liters:.1f} L ({result.tank_after_liters / tank.capacity_liters * 100:.1f}%)"
    )
    print("=" * 60)
    ...
    quick_result = quick_decision(
        zones=[{"zone_id": "z1", "name": "Test Zone", "area_m2": 1.0}],
        sensor_readings=[{"zone_id": "z1", "soil_moisture_percent": 20.0}],
